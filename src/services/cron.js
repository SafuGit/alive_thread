...
const startKeepAliveCron = (client, prisma) => {
  ...
  cron.schedule("0 0,12 * * *", async () => {
    keepAliveFn(client, prisma);
  });

  console.log("⏰ Keep-alive cron job scheduled (every 12 hours)");
};

const keepAliveFn = async (client, prisma) => {
  ...
  try {
    ...
    for (let i = 0; i < keepAliveThreads.length; i += BATCH_SIZE) {
      ...
      for (const keepAliveThread of batch) {
        ...
        console.log(
          `🔄 [${batchNum}/${totalBatches}] Processing: ${keepAliveThread.thread.name}`
        );
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, DELAY_BETWEEN_THREADS)
        );
      }
      ...
      if (updatedIds.length > 0) {
        ...
        console.log(
          `💾 Batch updated ${updatedIds.length} threads in database`
        );
    ...
    console.log(
      `✅ Keep-alive completed: ${successCount} success, ${failureCount} failures`
    );
  } catch (err) {
...
module.exports = { startKeepAliveCron, keepAliveFn };
